...
Since 0.7 the DeepSeek sessions run on [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (`dsh`), not on the connector's own loop. The connector starts one harness runtime per session through the official Python SDK (`deepseek-harness-sdk`, a bundled single-file runtime, JSON-RPC over stdio), with the session's workspace as `cwd` and an isolated home under `.local/dsh`. The harness owns the model loop — turns and steps, its file, shell, search and web tools, subagents, goals with autonomous rounds, compaction, prompt caching — and the connector translates its session events into the journal the interface already renders (`tool`, `tool_result`, `assistant`, `subagent`, `goal`, `usage`), delivers the final answer to Telegram, forwards a message sent mid-turn to the engine (harness 0.1.5 reads it right after the current turn and starts a follow-on turn, which the connector keeps following), and kills the runtime on stop.
...
### A Chrome window as a tool

`chrome_open`, `chrome_snapshot`, `chrome_click`, `chrome_type`, `chrome_press`, `chrome_upload`, `chrome_wait`, `chrome_screenshot` and `chrome_eval` drive a visible Chrome window over the DevTools protocol. It runs on AIGent's own profile directory (`.local/chrome-automation`) because Chrome 136+ ignores `--remote-debugging-port` on the user's default data directory, so the everyday Chrome is never touched; the owner signs in once, by hand, in the window the agent opens (`chrome_open` reports `sign_in_required` on a Google sign-in page), and that login persists there. Launching the window is an approval. Snapshots list the interactive elements with refs and walk shadow DOM (YouTube Studio is Polymer through and through); clicks are real mouse events at the element's centre; `chrome_upload` puts a file into a file input without an OS dialog (approval, absolute paths allowed); `chrome_screenshot` reaches a vision-capable model as an image. The protocol client is a hundred lines of WebSocket in `connector/chrome_tools.py`, no Playwright. Settings: `chrome_debug_port` (9333), `chrome_user_data_dir`, `chrome_profile`. The `youtube-upload` skill describes the YouTube Studio flow the agent follows.
