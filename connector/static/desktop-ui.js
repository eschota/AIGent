...
for(const [text,action] of [['Файлы',()=>showDock('editor')],['Git',()=>showDock('git')],['Терминал',()=>showDock('terminal')],['⋯',showSessionMenu]]){const b=el('button',text);b.onclick=handle(action);headerActions.append(b);}
...
async function loadTree(){const files=await api(`/api/sessions/${current.id}/files`);$('file-tree').replaceChildren(...files.map(f=>{const b=el('button',f.path);b.title=f.path;b.onclick=handle(()=>openEditor(f.path));return b;}));}
